class ExceptionalArticle < ApplicationRecord
    ...
    begin
      ...
      ExceptionalArticle.transaction do
        data.each do |row|
        	i += 1
          row_number = (i+1)
          move_to_next = false
          ...
          if row["ScanInd"].blank?
            error_found = true
            error_row = prepare_error_hash(row, row_number, "Scan Indicator is not present")
            errors_hash[row_number] << error_row
            move_to_next = true
          end
          if (row["ScanInd"].present? && ((row["ScanInd"] != "Y") || (row["ScanInd"] != "N")))
            error_found = true
            error_row = prepare_error_hash(row, row_number, "Scan Indicator should have values of Y / N")
            errors_hash[row_number] << error_row
            move_to_next = true
          end
		      ...
		      if row["ScanInd"].present?
		      	if row["ScanInd"].strip == "Y"
		      		scan_id = "Y"
		      	elsif row["ScanInd"].strip == "N"
		      		scan_id = "N"
		      	else
		      		scan_id = nil
		      	end
		      end
          exceptional_article = ExceptionalArticle.where("sku_code = ?", format('%018d', "#{row['Article'].try(:strip)}")).last
		      if exceptional_article.present?
		      	exceptional_article.update(scan_id: scan_id)
		      else
		      	self.create(sku_code: format('%018d', "#{row['Article'].try(:strip)}"),scan_id: scan_id)
		      end
	...
	def self.create_article_scan_ind_mapping(master_data_input_id)
    ...
    failed_count = 0
      ...
      begin
        master_data.payload["payload"].each do |data|
        	exceptional_article = ExceptionalArticle.where("sku_code ilike (?)", "%#{data['sku_code']}").last
        	if (exceptional_article.blank? && data['sku_status_ind'] == "A")
            exceptional_article = ExceptionalArticle.new(sku_code: data['sku_code'], scan_id: data['scan_id'], master_data_input_id: master_data.id)
            ...
            if (data['sku_status_ind'] == "A" && exceptional_article.update(sku_code: data['sku_code'], scan_id: data['scan_id'], master_data_input_id: master_data.id))
              success_count = success_count + 1
