class Tally::OutwardReturnOrderService
  include Utils::Formatting

  def self.get_records(data:)
    values_arr = data.map do |oms|
      {
        orrd_number: oms.reason_reference_document_no,
        orrd_date: oms.rrd_creation_date,
        status: oms.status,
        ...
        orrd_account_name: oms.details["so_account_name"],
