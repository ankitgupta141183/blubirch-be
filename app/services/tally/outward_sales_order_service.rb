class Tally::OutwardSalesOrderService
  include Utils::Formatting

  def self.get_records(data:)
    values_arr = data.map do |oms|
      {
        so_number: oms.reason_reference_document_no,
        so_date: oms.rrd_creation_date,
        status: oms.status,
        ...
        so_account_name: oms.details["so_account_name"],
