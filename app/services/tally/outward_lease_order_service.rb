class Tally::OutwardLeaseOrderService
  include Utils::Formatting

  def self.get_records(data:)
    values_arr = data.map do |oms|
      {
        lo_number: oms.reason_reference_document_no,
        lo_date: oms.rrd_creation_date,
        status: oms.status,
        ...
        lo_account_name: oms.details["so_account_name"],
