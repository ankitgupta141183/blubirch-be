class Tally::OutwardReplacementCustomerOrderService
  include Utils::Formatting

  def self.get_records(data:)
    values_arr = data.map do |oms|
      {
        rco_number: oms.reason_reference_document_no,
        rco_date: oms.rrd_creation_date,
        status: oms.status,
        ...
        rco_account_name: oms.details["so_account_name"],
