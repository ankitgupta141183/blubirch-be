class Tally::OutwardReplacementOrderService
  include Utils::Formatting

  def self.get_records(data:)
    values_arr = data.map do |oms|
      {
        rpo_number: oms.reason_reference_document_no,
        rpo_date: oms.rrd_creation_date,
        status: oms.status,
        ...
        rpo_account_name: oms.details["so_account_name"],
