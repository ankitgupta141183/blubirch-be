class Tally::OutwardRepairOrderService
  include Utils::Formatting

  def self.get_records(data:)
    values_arr = data.map do |oms|
      {
        rro_number: oms.reason_reference_document_no,
        rro_date: oms.rrd_creation_date,
        status: oms.status,
        ...
        rro_account_name: oms.details["so_account_name"],
