class Api::V1::Store::PendingDispatchController < ApplicationController
  ...
  def create_consignment
    ...
    if @consignment.save
      inventory_status_store_in_transit = LookupValue.where(code: Rails.application.credentials.inventory_status_store_in_transit).first
      ...
      params['gate_pass_ids'].each do |id|
        ...
        inventories = Inventory.where("details ->> 'dispatch_gatepass_number' = ?", gate_pass.gatepass_number)
        inventories.each do |inventory|
          ...
          new_inventory_status.status_id = inventory_status_store_in_transit.try(:id)
          new_inventory_status.is_active = true          
          if new_inventory_status.save
            last_inventory_status.update(is_active: false)
            inventory.update(details: inventory.merge_details({"store_dispatch_date" => Time.now.to_s, "store_dispatch_user" => current_user.try(:username), "status" => inventory_status_store_in_transit.try(:original_code)}))
          end
        end
      end
